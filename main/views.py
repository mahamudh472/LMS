...
def search(request):
    if request.method == "GET":
        ...
        books = Book.objects.filter(title__icontains=text) | Book.objects.filter(author__icontains=text) | Book.objects.filter(category__name__icontains=text) | Book.objects.filter(desc__icontains=text) 
        return render(request, "singlecata.html", {"books": books, "text": text, "count": books.count(), "operation": "search"})
